...
/* Initialization */ 
  song_node* test = insert_inOrder( "blank space", "taylor swift",0);
  insert_inOrder( "never", "taylor swift",test);
  insert_inOrder( "out song", "taylor swift",test);
  insert_inOrder( "work", "rhianna",test);
  insert_inOrder("singleladies", "beyonce",test); 
/* Testing find */
  printf("Testing find:\n"); 
  printf("t artists: "); 
  print_list(test);
...
/* Test random choosing*/
  ...
  print_single(random_node(test));
/* Testing remove_song*/ 
  test = remove_song("work", "rhianna",test);
